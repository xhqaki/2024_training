...
cursor.execute("alter table data_info add column label_lidar text")
for record in records:

    # 获取label_lidar_std_path字段的值（它是record的第7个元素）
    label_lidar_std_path = record[6]
    ...
    try:
        ...
        print(f"File not found: { label_lidar_std_path}")
        continue  # 跳过不存在的文件
    except Exception as e:
        print(f"Error reading file: {e}")
        continue  # 跳过有错误的文件

    update_query = " UPDATE data_info  SET  label_lidar = %s  WHERE id = %s  -- 假设id是主键，用于标识每一行  "
    try:
